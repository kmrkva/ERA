...
app.post("/chat", async (req, res) => {
  try {
    ...
    const contextPrompt = `Here is the current HTML code for a consumer research webpage that I'm working with:

\`\`\`html
${currentHtml}
\`\`\`

User request for changes: ${message}
...
- At the end of your response, include a brief summary of what changes you made

This is for consumer research experiments, so the webpage should look and function like a real eCommerce or consumer choice webpage.

Provide the complete updated HTML file followed by a summary of changes made.`
    ...
    let changesDescription = "Code has been updated based on your request."
    const responseText = response.text

    // Try to extract a summary of changes from the response
    const summaryMatch = responseText.match(/(?:changes made|summary|modifications):\s*(.+?)(?:\n|$)/i)
    if (summaryMatch) {
      changesDescription = summaryMatch[1].trim()
    }
